...
<?php 
//var_dump($selectdata);
foreach ($datamal as $row) 
?>
<br></br>
<h1>Halaman Edit Data Muzakki Zakat Mal</h1>

<p>Masukkan Data Dengan Benar</p>
<form action="<?php echo base_url('Datamal/saveEditDataMal/' .$row->nik.'')?>" method="post" form class="pure-form pure-form-aligned">
    <fieldset>
        ...
        <div class="pure-control-group">
            <label for="jml_hasil">JUMLAH PENGHASILAN</label>
            <input type="text" id="jml_hasil" name="jml_hasil" onkeyup="hitungPersen()" value="<?php echo $row->jml_hasil;?>"/>
        </div>
        <div class="pure-control-group">
            <label for="jml_zakat">JUMLAH ZAKAT</label>
            <input type="text" id="jml_zakat" name="jml_zakat" readonly value="<?php echo $row->jml_zakat;?>"/>
        </div>
    ...
    function hitungPersen() {
			// Mendapatkan nilai dari input jumlah
			var jml_hasil = document.getElementById("jml_hasil").value;

			// Menghitung 2.5% dari jumlah
			var jml_zakat = jml_hasil * 0.025;

			// Menetapkan nilai persen ke input persen
			document.getElementById("jml_zakat").value = jml_zakat;
		}
	</script> 
  <script>
		function cekPanjang() {
